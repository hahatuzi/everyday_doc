...
1. [项目工程规范化范围](#一、项目工程规范化范围)
2. [eslint](#二、eslint)
3. [prettier](#三、prettier)
4. [stylelint](#四、stylelint)
5. [husky](#五、husky)
6. [lint-staged](#六、lint-staged)
7. [commitlint](#七、commitlint)
...
```
  ──→ 代码输入
  ──→ 读取Eslint配置文件
  ──→ 选择适用的解析器(比如typescript-parser,babelParser)
  ──→ 生成AST
  ──→ 遍历AST节点
  ──→ 应用已经配置的规则
  ──→ 节点是否符合规则
...
- 第三步：`**npx eslint --init**`生成配置文件`.eslint.cjs`或者`eslint.config.js`
- 第四步：解决`npm run lint`时报错`SyntaxError: Unexpected token 'export',package.json添加"type": "module"`
...
- 第七步：在`eslint.config.js`中配置`**@typescript-eslint/parser**`
- 第八步：针对vue文件进行校验,`**npm i vue-eslint-parser**`
- 第九步：添加.eslintignore忽略文件,文件内容如下：
  ```js
  // dist
  // node_modules
  // public
...
- 直接使用eslint的集成插件：`**npm init @eslint/config@latest**`，就会直接在项目中生成`eslint.config.js`，并自动安装集成工具@eslint/js
- 方式二中使用typescript校验的方式：`typescript-eslint`插件，而不是`@typescript-eslint/parser`该插件
...
#### 方式一：通过使用prettier插件实现格式化,本地运行时!!!无关eslint和CICD流程
...
#### 方式二：通过在eslint中使用插件prettier实现格式化
...
### 3.3 为什么需要eslint-plugin-prettier配合使用eslint-config-prettier？

单独使用eslint-plugin-prettier可能会导致ESLint和Prettier**规则冲突**。例如，ESLint的indent规则和Prettier的缩进规则可能不一致，导致格式化结果不符合预期。这时候就需要`eslint-config-prettier`来关闭ESLint中与Prettier冲突的规则。
...
> 利用husky在代码提交之前触发`git hook`(git在客户端的钩子)，然后执行`pnpm run format`来自动的格式化我们的代码。
...
// 第二步：安装husky git hooks
  // 方式一：
  npx husky-init // 该命令会在根目录下生成一个.husky目录，在这个目录下会有一个pre-commit文件，该文件中的命令会在我们执行commit的时候执行
  // 方式二：
  先配置package.json,scripts: "prepare":"husky inastall"再npm run prepare
// 第三步在.husky/pre-commit文件添加如下命令：
  #!/usr/bin/env sh
  . "$(dirname -- "$0")/_/husky.sh"
  npm run format
// 第四步：测试脚本：测试husky钩子，添加pre-commit钩子
...
> lint-staged:是一个在`git add`到暂存区的文件运行linter的工具，避免在`git commit`提交时在整个项目执行，它实现了自动对将要提交的文件执行`eslint --fix`和`prettier --write`。
...
> 安装commitlint以及相关插件检查提交消息是否符合`commit format`
